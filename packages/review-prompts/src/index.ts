import { mergeBaseWithHead } from '@review-agent/review-git';
...
export const REVIEW_PROMPT_PACK_ID = 'codex-review-parity-v1';
...
export async function reviewPrompt(
  ...
  switch (target.type) {
    ...
    case 'baseBranch': {
      ...
      if (mergeBaseSha) {
        return BASE_BRANCH_PROMPT.replaceAll(
          '{baseBranch}',
          target.branch
        ).replaceAll('{mergeBaseSha}', mergeBaseSha);
      }
      return BASE_BRANCH_PROMPT_BACKUP.replaceAll('{branch}', target.branch);
    }
    case 'commit':
      if (target.title) {
        return COMMIT_PROMPT_WITH_TITLE.replaceAll(
          '{sha}',
          target.sha
        ).replaceAll('{title}', target.title);
      }
      return COMMIT_PROMPT.replaceAll('{sha}', target.sha);
    case 'custom': {
...
export async function resolveReviewRequest(
  request: ResolveReviewRequestInput,
  cwd: string
): Promise<ResolvedReviewRequest> {
  const prompt = await reviewPrompt(request.target, cwd);
  return {
    target: request.target,
    prompt,
    userFacingHint: request.userFacingHint ?? userFacingHint(request.target),
  };
}
